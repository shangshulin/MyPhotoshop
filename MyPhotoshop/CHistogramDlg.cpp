...
void CHistogramDlg::OnPaint()
{
    ...
    else if (m_histogramType == 0)
    {
        ...
        // 获取最大值
        int maxValue = *std::max_element(m_histogramData.begin(), m_histogramData.end());
        ...
        for (int i = 0; i <= numTicks; ++i)
        {
            ...
            //// 显示刻度标签（频数值形式）
            //double value = static_cast<double>(i) / numTicks * maxValue; // 频数值
            //CString strLabel;
            //strLabel.Format(_T("%.0f"), value); // 格式化为字符串
            //dc.TextOutW(plotRect.left - 40, yPos - 10, strLabel); // 在刻度旁显示标签
            
            // 显示刻度标签（归一化形式）
            double normalizedValue = static_cast<double>(i) / numTicks * maxValue / totalPixels; // 归一化值
            CString strLabel;
            strLabel.Format(_T("%.4f"), normalizedValue); // 格式化为字符串
            dc.TextOutW(plotRect.left - 40, yPos - 10, strLabel); // 在刻度旁显示标签
        }
        ...
        {
            // 将数据值映射到绘图区域的高度
            int barHeight = static_cast<int>(std::round(static_cast<double>(m_histogramData[i]) * plotRect.Height() / maxValue));
    ...
    else if (m_histogramType == 1)
    {
        ...
        int maxValue = max(maxRed, max(maxGreen, maxBlue));
        ...
        {
            // 将数据值映射到绘图区域的高度
            int redBarHeight = static_cast<int>(std::round(static_cast<double>(m_histogramDatas[0][i]) * plotRect.Height() / maxValue));
            int greenBarHeight = static_cast<int>(std::round(static_cast<double>(m_histogramDatas[1][i]) * plotRect.Height() / maxValue));
            int blueBarHeight = static_cast<int>(std::round(static_cast<double>(m_histogramDatas[2][i]) * plotRect.Height() / maxValue));
